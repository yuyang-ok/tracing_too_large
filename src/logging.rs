...
///   DATAFABRIC_JAEGER_AGENT_ENDPOINT=localhost:6831 RUST_LOG=trace cargo test
///   open http://localhost:16686/
///
/// To adjust batch sending delay, use `OTEL_BSP_SCHEDULE_DELAY`:
/// DATAFABRIC_JAEGER_AGENT_ENDPOINT=localhost:6831 RUST_LOG=trace OTEL_BSP_SCHEDULE_DELAY=1 cargo test
/// TODO(xp): use DATAFABRIC_JAEGER_AGENT_ENDPOINT to assign jaeger server address.
///
...
pub fn init_logging(name: &str, cfg: &Config) -> Vec<WorkerGuard> {
    ...
    let jaeger_agent_endpoint =
        env::var("DATAFABRIC_JAEGER_AGENT_ENDPOINT").unwrap_or_else(|_| "".to_string());
    ...
    let tracing_layer = if env::var("DATAFABRIC_TRACING_LOG_ENABLED").is_ok() {
    ...
    let bend_sentry_env = env::var("DATAFABRIC_SENTRY_DSN").unwrap_or_else(|_| "".to_string());
...
impl QueryLogger {
    pub fn init(app_name_shuffle: String, config: &Config) -> Result<(), ()> {
        let app_name = format!("datafabric-query-{}", app_name_shuffle);
